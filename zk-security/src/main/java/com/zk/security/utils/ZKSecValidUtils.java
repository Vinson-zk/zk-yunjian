...
public class ZKSecValidUtils {
    ...
    private static Logger logger = LoggerFactory.getLogger(ZKSecValidUtils.class);
     ...
     * @return
     */
    public static String getCaptchaIdByRequest(ServletRequest request) {
        // 先从请求参数中取 captchaId,没有再从中请求头中取
        String captchaId = ZKWebUtils.getCleanParam(request, ZKSecConstants.PARAM_NAME.CaptchaId);
        // captchaId 为空，再从中请求头中取
        if (ZKStringUtils.isEmpty(captchaId)) {
            captchaId = ZKWebUtils.toHttp(request)
                    .getHeader(ZKSecConstants.PARAM_NAME.CaptchaId);
        }
        return captchaId;
    }
}
