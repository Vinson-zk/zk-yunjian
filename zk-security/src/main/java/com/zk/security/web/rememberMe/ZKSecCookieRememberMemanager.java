...
public class ZKSecCookieRememberMemanager extends ZKSecAbstractRememberMeManager {
    ...
    protected void forgetIdentity(ZKSecSubject subject) {
        if (subject instanceof ZKSecWebSubject) {
            HttpServletRequest request = ((ZKSecWebSubject) subject).getRequest();
            HttpServletResponse response = ((ZKSecWebSubject) subject).getResponse();
     ...
     * @return
     */
    private boolean isIdentityRemoved(HttpServletRequest request, HttpServletResponse response) {
        if (request != null) {
