...
public interface ZKSysAuthFuncApiDao extends ZKBaseDao<String, ZKSysAuthFuncApi> {
    ...
    // 查询权限拥有的API接口代码
    @Select(" SELECT c_func_api_code FROM ${tn} WHERE c_auth_id = #{authId} and c_del_flag = #{delFlag} ")
    List<String> findApiCodesByAuthId(@Param("tn") String tn, @Param("authId") String authId,
            @Param("delFlag") int delFlag);

    // 查询权限拥有的API接口ID
    @Select(" SELECT c_func_api_id FROM ${tn} WHERE c_auth_id = #{authId} and c_del_flag = #{delFlag} ")
