...
public interface ZKSysAuthUserRoleDao extends ZKBaseDao<String, ZKSysAuthUserRole> {
    ...
    List<String> findApiCodesByUserId(@Param("userRoleTb") String userRoleTb, @Param("authRoleTb") String authRoleTb,
            @Param("authApiTb") String authApiTb, @Param("userId") String userId, @Param("delFlag") int delFlag);
    ...
    // 查询用户拥有的角色代码
    @Select(" SELECT ${tAlias}.c_role_code FROM ${tn} ${tAlias} WHERE ${tAlias}.c_user_id = #{userId} and ${tAlias}.c_del_flag = #{delFlag} ")
    List<String> findRoleCodesByUserId(@Param("tn") String tn, @Param("tAlias") String tAlias,
            @Param("userId") String userId, @Param("delFlag") int delFlag);
     ...
     * @return List<String>
     */
    @Select(" SELECT ${tAlias}.c_role_id FROM ${tn} ${tAlias} WHERE ${tAlias}.c_user_id = #{userId} and ${tAlias}.c_del_flag = #{delFlag} ")
