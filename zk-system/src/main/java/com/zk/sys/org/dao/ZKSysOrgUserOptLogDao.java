...
* with ZK-Vinson. 
*
* @Title: ZKSysOrgUserOptLogDao.java 
* @author Vinson 
...
/**
 * @ClassName: ZKSysOrgUserOptLogDao
 * @Description: TODO(simple description this class what to do. )
 * @author Vinson
 * @version 1.0
 */
@ZKMyBatisDao
public interface ZKSysOrgUserOptLogDao extends ZKBaseDao<String, ZKSysOrgUserOptLog> {


    @Select(value = {
            "SELECT ${sCols} FROM ${tn} ${alias} WHERE ${alias}.c_user_id = #{userId} AND ${alias}.c_edit_type = #{editType} ORDER BY ${alias}.c_create_date DESC " })
    List<ZKSysOrgUserOptLog> findByEditType(@Param("tn") String tn, @Param("alias") String alias,
