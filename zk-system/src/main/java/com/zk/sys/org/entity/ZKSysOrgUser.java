...
public class ZKSysOrgUser extends ZKBaseEntity<String, ZKSysOrgUser> implements ZKUser<String> {
    ...
    @NotNull(message = "{zk.core.data.validation.notNull}")
    @Length(min = 1, max = 64, message = "{zk.core.data.validation.length.max}")
    @ZKColumn(name = "c_password", isInsert = true, javaType = String.class)
    String password;
    /**
     * 密码状态；0-系统密码；1-用户密码；
     */
    ...
    @ZKColumn(name = "c_pwd_status", isInsert = true, javaType = Long.class)
    Integer pwdStatus;
    ...
    @ZKColumn(name = "c_pwd_last_update_date", isInsert = true, javaType = Date.class, formats = "%Y-%m-%d %H:%i:%S")
    ...
    @NotNull(message = "{zk.core.data.validation.notNull}")
    @Length(min = 1, max = 64, message = "{zk.core.data.validation.length.max}")
    @ZKColumn(name = "c_family_name", isInsert = true, javaType = String.class, update = @ZKUpdate(true),
        query = @ZKQuery(queryType = ZKDBOptComparison.LIKE))
    String familyName;
    /**
     * 名
