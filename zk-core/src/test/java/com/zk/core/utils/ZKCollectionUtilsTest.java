...
public class ZKCollectionUtilsTest {
    ...
    public void testMapToReqParametMap() {
        try {
            ...
            String jsonStr = ZKJsonUtils.writeObjectJson(sourceMap);
            System.out.println("[^_^:20190625-1735-001] jsonStr: " + jsonStr);

            targetMap = ZKCollectionUtils.mapToReqParametMap(ZKJsonUtils.jsonStrToMap(jsonStr));
//            targetMap = ZKCollectionUtils.mapToReqParametMap(sourceMap);
            ...
            TestCase.assertEquals(ZKJsonUtils.writeObjectJson(eListE1), targetMap.get(test_pn.pn_entityList)[0]);
            TestCase.assertEquals(ZKJsonUtils.writeObjectJson(eListE2), targetMap.get(test_pn.pn_entityList)[1]);
            TestCase.assertEquals(ZKJsonUtils.writeObjectJson(eArrayE1), targetMap.get(test_pn.pn_entityArray)[0]);
            TestCase.assertEquals(ZKJsonUtils.writeObjectJson(eArrayE2), targetMap.get(test_pn.pn_entityArray)[1]);
            TestCase.assertEquals(ZKJsonUtils.writeObjectJson(eMap), targetMap.get(test_pn.pn_entityMap)[0]);
