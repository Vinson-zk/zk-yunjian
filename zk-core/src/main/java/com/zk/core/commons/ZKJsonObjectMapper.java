...
public class ZKJsonObjectMapper extends ObjectMapper {
    ...
    private static Logger logger = LogManager.getLogger(ZKJsonObjectMapper.class);

    private static ZKJsonObjectMapper mapper;

    private static ZKJsonObjectMapper nonMapper;
...
//                jgen.writeString(HtmlUtils.unescapeHtml4(value));
    ...
    public static ZKJsonObjectMapper nonDefaultMapper() {
        if (nonMapper == null) {
            nonMapper = new ZKJsonObjectMapper(Include.NON_NULL);
            initMapper(nonMapper);
        }
        return nonMapper;
    }

    public static ZKJsonObjectMapper newMapper() {
        ZKJsonObjectMapper mapper = new ZKJsonObjectMapper(Include.NON_NULL);
        initMapper(mapper);
        return mapper;
    }

    private static void initMapper(ZKJsonObjectMapper mapper) {
        // 现在需要在中间加一个
    ...
    public String toJson(Object object) {
        try {
            ...
            logger.warn("write to json string error:", e.getMessage());
            e.printStackTrace();
