...
//            log.info("[^_^:20190627-1031-003-1] 解密后 reqBody -> " + new String(reqBody));
            ...
            throw new ZKUnknownException("解密 request body 失败!", e);
        } finally {
...
//        log.info("[^_^:20190625-1540-002-1] 解密后 数据：paramStr -> " + paramStr);
    ...
    public void encrypt(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (request instanceof ZKRequestWrapper && response instanceof ZKResponseWrapper) {
            ...
            // 再抛个异常比较好
            log.error("[^_^:20190627-1121-001] 传输加解密使用工具不对应。");
            throw new ZKUnknownException("传输加解密使用工具不对应。");
        }
    }
    ...
    public ZKRSAKey getZKRSAKeyByRsaId(String rsaId) {
        try {
            ...
            throw new ZKUnknownException("取 RSA 证书失败", e);
        }
    ...
    private byte[] decryptData(byte[] encKey, byte[] salt, byte[] content) {
        ...
        try {
            ...
            throw new ZKUnknownException("AES 解密数据失败", e);
        }
    }
    ...
    public byte[] encryptData(byte[] encKey, byte[] content) {
        try {
            ...
            throw new ZKUnknownException("AES 响应数据加密 失败", e);
        }
    }
    ...
    private byte[] decryptDataKey(byte[] privateKey, byte[] encKey) {
        try {
            ...
            throw new ZKUnknownException("RSA 解密数据加密 密钥 失败", e);
        }
    }
    ...
    public byte[] encryptDataKey(byte[] publicKey, byte[] encKey) {
        try {
            ...
            throw new ZKUnknownException("RSA 加密数据加密 密钥 失败", e);
        }
    }
