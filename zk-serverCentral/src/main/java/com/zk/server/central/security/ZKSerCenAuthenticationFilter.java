...
//        HttpServletRequest hReq = (HttpServletRequest) request;
        ...
        HttpServletRequest hReq = ZKWebUtils.toHttp(request);
...
//            throw new ZKSerCenAuthenticationException("zk.ser.cen.000016", null, null, e);
        ...
        if (isRememberAccount(request)) {
            javax.servlet.http.Cookie cookieAccount = ZKCookieUtils.createCookie(ZKAuthKeys.PARAM_REMEMBER_ACCOUNT,
                    account, null);

            ZKCookieUtils.saveToHeader(ZKWebUtils.toHttp(response), cookieAccount);
        }
        else {
            ZKCookieUtils.removeCookie(ZKWebUtils.toHttp(request), ZKWebUtils.toHttp(response),
