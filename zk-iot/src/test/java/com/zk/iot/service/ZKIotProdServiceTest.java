...
/**
 * ZKIotProdServiceTest
 * @author 
 * @version 
 */
public class ZKIotProdServiceTest {

    // =======================================================
    // 需要提前准备好测试数据：t-code-cate 场景
    // 需要提前准备好测试数据：t-code-brand 品牌
    // =======================================================

    static final String cateCode = "cateNet";

    static final String brandCode = "brandNet";

	static ZKIotProd makeNew() {
        ZKIotProd e = new ZKIotProd();
        e.setVersion(-1l);
        e.setRemarks("zk.test.data");
        // e.set
        e.setBrandCode(brandCode);
        e.setCateCode(cateCode);
        e.setProdCode("t-code-prod-temp");
        e.setProdName(ZKJson.parse("{\"zh_CN\":\"测试产品1\"}"));
	...
	@Test
    public void testDml() {
        ZKIotProdService s = ZKIotTestHelper.getMainCtx().getBean(ZKIotProdService.class);
        List<ZKIotProd> dels = new ArrayList<>();
        ...
        try {
            ...
            /*** 修改 ***/
            result = 0;
            e.setProdName(ZKJson.parse("{\"zh_CN\":\"测试产品1-update\"}"));
            result = s.save(e);
            TestCase.assertEquals(1, result);
            e = s.get(e);
            TestCase.assertNotNull(e);
            TestCase.assertEquals(ZKJson.parse("{\"zh_CN\":\"测试产品1-update\"}").toString(), e.getProdName().toString());
            ...
            try {
                ...
                TestCase.assertEquals("zk.iot.000004", ex.getCode());
            }
