...
@ZKTable(name = "t_iot_log_func_record", alias = "iotLogFuncRecord", orderBy = " c_create_date ASC ")
public class ZKIotLogFuncRecord extends ZKBaseEntity<String, ZKIotLogFuncRecord> {
    ...
    public static ZKDBSqlHelper sqlHelper() {
        if (sqlHelper == null) {
            sqlHelper = new ZKDBSqlHelper(new ZKSqlConvertDelegating(), new ZKIotLogFuncRecord());
        }
        return sqlHelper;
    }
	...
	@NotNull(message = "{zk.core.data.validation.notNull}")
	@Length(min = 1, max = 64, message = "{zk.core.data.validation.length.max}")
	@ZKColumn(name = "c_func_id", isInsert = true, javaType = String.class, update = @ZKUpdate(true), query = @ZKQuery(queryType = ZKDBOptComparison.EQ))
	String funcId;	
	/**
	 * 功能代码
	 */
	@NotNull(message = "{zk.core.data.validation.notNull}")
	@Length(min = 1, max = 64, message = "{zk.core.data.validation.length.max}")
	@ZKColumn(name = "c_func_code", isInsert = true, javaType = String.class, update = @ZKUpdate(true), query = @ZKQuery(queryType = ZKDBOptComparison.EQ))
	String funcCode;	
	/**
	 * 日志数据
	 */
	...
	public ZKIotLogFuncRecord() {
		super();
	}

    public ZKIotLogFuncRecord(String pkId) {
		super(pkId);
	}
