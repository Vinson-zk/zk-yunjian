...
public class ZKMailSendService {
    ...
    private boolean checkAttachmentCount(int length) {
        if (length > this.attachmentCount) {
            ...
            throw ZKCodeException.as("zk.mail.000010", "邮件附件最多允许[{}]个", null, this.attachmentCount);
//            return false;
        }
        return true;
    }
    ...
    private boolean checkAttachmentSize(long size) {
        if (size > this.attachmentSize) {
            ...
            throw ZKCodeException.as("zk.mail.000011", "邮件附件最大允许[{0}]M", null, (this.attachmentSize / 1024 / 1024));
        }
        return true;
    }
    ...
    protected void send(boolean async, String sendMailAddr, String recipientMailAddr, String sendFlag, String typeCode,
        ...
        if (mailType == null) {
            ...
            throw ZKCodeException.as("zk.mail.000004", "邮件类型不存在", null, typeCode);
        }

        if (mailType.getStatus().intValue() != ZKMailType.KeyStatus.normal) {
            log.error("[>_<:20220526-2058-002] zk.mail.000002=邮件类型[{}]被禁用；", typeCode);
            throw ZKCodeException.as("zk.mail.000002", "邮件类型被禁用", null, typeCode);
        }
        ...
        if (mailTemplate == null) {
            ...
            throw ZKCodeException.as("zk.mail.000007", "邮件模板[{0}][{1}][{2}]不存在", null, typeCode, companyCode, locale);
        }
        ...
        if (mailTemplate.getStatus().intValue() != ZKMailType.KeyStatus.normal) {
            ...
            throw ZKCodeException.as("zk.mail.000008", "邮件模板[{0}][{1}][{2}]已被禁用", null, typeCode, companyCode, locale);
        }
    ...
    protected void send(boolean async, String sendMailAddr, String recipientMailAddr, String sendFlag,
        ...
        try {
            ...
            e.printStackTrace();
            throw ZKCodeException.as(e, "zk.mail.000009", "发送邮件失败", null);
        }
    }
