...
* with ZK-Vinson. 
*
* @Title: ZKWebfluxTestControllerTest.java 
* @author Vinson 
* @Package com.zk.webflux.controller 
* @Description: TODO(simple description this file what to do. ) 
* @date Jan 24, 2024 6:51:14 PM 
* @version V1.0 
*/
package com.zk.webflux.controller;
...
import com.zk.webflux.helper.controller.ZKWebfluxTestController;
import com.zk.webflux.helper.entity.ZKWebfluxTestEntity;

import junit.framework.TestCase;

/** 
* @ClassName: ZKWebfluxTestControllerTest 
...
//@WebFluxTest
public class ZKWebfluxTestControllerTest {
...
//            for (HttpMessageConverter<?> o : template.getRestTemplate().getMessageConverters()) {
//                System.out.println(
...
//            }
            ...
            System.out.println("[^_^:20190624-1022-001] ZKWebfluxTestControllerTest.index: " + resStr);
            TestCase.assertEquals(ZKWebfluxTestController.msg_index, resStr);
        }
        catch(Exception e) {
            e.printStackTrace();
            TestCase.assertTrue(false);
    ...
    @Test
    public void testIndexOnWebTestClient() {
        try {

//            Mockito.when(repository.findById(100)).thenReturn(Mono.just(employee));
//
//            webClient.get().uri("/{id}", 100).exchange().expectStatus().isOk().expectBody().jsonPath("$.name")
//                    .isNotEmpty().jsonPath("$.id").isEqualTo(100).jsonPath("$.name").isEqualTo("Test")
//                    .jsonPath("$.salary").isEqualTo(1000);
//
//            Mockito.verify(repository, times(1)).findById(100);

            ResponseSpec responseSpec;
            BodySpec<String, ?> bodySpec;
            EntityExchangeResult<String> entityExchangeResult;
            String resStr = "";
//            Mono<String> responseMono;

            responseSpec = webTestClient.get().uri(this.baseUrl).exchange();
            responseSpec.expectStatus().isOk();
            bodySpec = responseSpec.expectBody(String.class);
            entityExchangeResult = bodySpec.returnResult();
            resStr = entityExchangeResult.getResponseBody();
            System.out.println("[^_^:20190624-1022-011] ZKWebfluxTestControllerTest.index: " + resStr);
            TestCase.assertEquals(ZKWebfluxTestController.msg_index, resStr);
        }
        catch(Exception e) {
            e.printStackTrace();
    ...
    public void testGet() {
        try {
            ...
            resStr = response.getBody();
            System.out.println("[^_^:20230131-0858-011] resStr: " + resStr);
            TestCase.assertEquals(ZKWebfluxTestController.msg_index + param, resStr);
    ...
    public void testZkMsgRes() {
        try {
            ...
            TestCase.assertEquals(200, resStatusCode);
            resStr = response.getBody();
            System.out.println("[^_^:20230131-0859-023] resStr: " + resStr);
            ...
            TestCase.assertEquals(200, resStatusCode);
            resStr = response.getBody();
            System.out.println("[^_^:20230131-0859-022] resStr: " + resStr);
            ms = ZKJsonUtils.parseObject(resStr, ZKMsgRes.class);
            TestCase.assertEquals("zk.000003", ms.getCode());
            TestCase.assertEquals("项目环境配置错误。", ms.getMsg());
            TestCase.assertEquals(param, ms.getDataStr());
    ...
    public void testPostJson() {
        try {
            ...
            entity = new ZKWebfluxTestEntity();
            entity.setName("name 三尺剑，六钧弓，岭北对江东。");
            entity.setAge(99);
            param = "你回眸一笑，三秋却如一日。";
            url = baseUrl + "/postJson?param=" + param;

            requestEntity = new HttpEntity<ZKWebfluxTestEntity>(entity, headers);
...
//            TestCase.assertEquals(ZKJsonUtils.toJsonStr(entity), resStr);
            ...
            te = ZKJsonUtils.parseObject(resStr, ZKWebfluxTestEntity.class);
